...
def execute(filters=None):
	...
	total_salary_map = get_salary_structure_assignment_map(salary_slips)
	...
	for ss in salary_slips:
		row = {
			...
			"custom_total_salary": total_salary_map.get(ss.employee),
		}
...
def get_salary_structure_assignment_map(salary_slips):
	"""
	Har employee ki Salary Structure Assignment (submitted) me se
	custom_total_salary uthata hai. Agar kisi employee ki ek se zyada
	assignments hon, to sabse latest (from_date ke hisab se) use hoti hai.
	Returns: {employee: custom_total_salary}
	"""
	employees = list({ss.employee for ss in salary_slips})
	if not employees:
		return {}
	...
	assignments = (
		frappe.qb.from_(ssa)
		.where((ssa.employee.isin(employees)) & (ssa.docstatus == 1))
		.select(ssa.employee, ssa.from_date, ssa.custom_total_salary)
		.orderby(ssa.from_date)
	).run(as_dict=1)

	result = {}
	for a in assignments:
		# list from_date ascending hai, isliye har baar overwrite karne se
		# sabse latest wali value bach jayegi
		result[a.employee] = flt(a.custom_total_salary)

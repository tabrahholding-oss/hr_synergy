...
class FinancialOverview {
	...
	setup_filters() {
		...
		Promise.all([
			...
			console.error(
				'Financial Overview filters failed to load',
				error
			);
	...
	create_filter_bar() {
		...
		// Remove old filter bar if page is hot reloaded
		$(this.page.wrapper)
			.find('.fo-filter-bar')
			.remove();
		...
		this.$filter_bar = $(`
			<div class="fo-filter-bar">
				...
				<div class="fo-filter-field fo-filter-company">
					...
					<div class="fo-filter-label">
						${__('Company')}
					</div>

					<div
						class="fo-filter-control"
						data-field="company"
					>
						<select
							class="fo-native-select"
							id="fo-company-select"
						>
							<option value="">
				...
				<div class="fo-filter-field fo-filter-fiscal-year">
					...
					>
						<select
							class="fo-native-select"
							id="fo-fiscal-year-select"
						>
							<option value="">
								${__('Loading fiscal years...')}
							</option>
						</select>
					</div>
		...
		const $page_head = $(this.page.wrapper)
			.find('.page-head')
			.first();
		...
		// Keep same API used by rest of class
		...
		this.fiscal_year_control = {
			...
			get_value: () => {
				return $fiscal_year.val() || '';
			},

			set_value: value => {
				$fiscal_year.val(value || '');
			}
		...
		frappe.call({
			...
			callback: (r) => {
				...
				if (!r.message) {
					...
					console.error(
						'Financial Overview returned no data',
						r
					);
				...
				try {
					...
					console.error(
						'Financial Overview failed while rendering',
						error,
						r.message
					);
	...
	render_error(error) {
		...
		$(this.page.body).html(`
			<div class="fo-page fo-loading-state">
				...
				<div class="fo-loading-text">
					${__(
						'Could not load financial data. Please check the server logs.'
					)}
				</div>
		...
		const stat_cards_html =
			d.stat_cards.map(c => `
				...
				<div class="fo-card">
					...
					<div class="fo-card-label">
						${c.label}
					</div>


					<div class="fo-card-value">
						${c.value_fmt}
					</div>


					<div class="fo-card-vs">
							...
							(q, i) => `
								...
								>
									...
									<div
										...
										style="height:${q.bar_pct}%"
									></div>

									<div class="fo-bar-label">
										${q.label}
									</div>
			...
			<div class="fo-page">
				...
				<div class="fo-stats-row">
					...
					<div class="fo-hero-card">
						...
						<div class="fo-hero-value">
							${d.total_revenue.value_fmt}
						</div>
						...
						<div class="fo-hero-metrics">
							...
							<div>
								...
								<div class="fo-hero-metric-value">
									${d.total_revenue.gross_margin}%
								</div>
						...
						<div class="fo-breakdown-body">
							...
							<div class="fo-donut-wrap">
								...
								<div class="fo-donut-center">
									...
									<div class="fo-donut-total-label">
										${__('Total')}
									</div>

									<div class="fo-donut-total-value">
										${d.revenue_total_fmt}
									</div>
	...
	render_donut(breakdown) {
		...
		const highlight = (i) => {
			...
			$svg
				.find(
					`.fo-donut-slice[data-slice="${i}"]`
				)
				.addClass(
					'fo-donut-slice-active'
				);
		...
		const unhighlight = () => {
			...
			$svg
				.find('.fo-donut-slice')
				.removeClass(
					'fo-donut-slice-dim fo-donut-slice-active'
				);
		};
			...
			(i, target_rect) => {
				...
				const html = `
					...
					<div class="fo-popup-total-row">
						...
						<span class="fo-popup-total-value">
							${seg.value_fmt}
						</span>

						<span class="fo-popup-total-pct">
							${seg.pct}%
						</span>
					...
					<div class="fo-popup-sub-heading">
						${__('BREAKDOWN')}
					</div>
					...
					${
						sub_html ||
						`
							<div class="fo-popup-sub-empty">
								${__('No item-level data')}
							</div>
						`
					}
	...
	render_trend_chart(trend) {
		...
		const svg = `
			...
			<div class="fo-trend-legend">
				...
				<span>
					<i
						class="fo-line-swatch"
						style="background:#1c6b4a"
					></i>
		...
		$overlay.on(
			...
			(e) => {
				...
				$('.fo-axis-label[id^="fo-month-label-"]')
					.removeClass(
						'fo-axis-label-active'
					);
				...
				const html = `
					...
					<div class="fo-trend-tip-row">
						...
						<b>
							${currency_symbol}${p.value.toFixed(1)}M
						</b>
					...
					<div class="fo-trend-tip-row">
						...
						<span
							class="fo-dot fo-dot-hollow"
							style="border-color:#8fc9a9"
						></span>

						${__('Prior Year')}

						<b>
							${currency_symbol}${p.prior.toFixed(1)}M
						</b>
					...
					<div class="fo-trend-tip-yoy">
						...
						<span class="fo-trend-tip-amount">
							(${p.change_amount_fmt})
						</span>
		...
		.on(
			...
			() => {
				...
				$(
					'#fo-crosshair, #fo-dot-current, #fo-dot-prior'
				)
					.hide();

...
def get_dashboard_payload(data):
    ...
    total_invoices = len(set([row.get("sales_invoice") for row in data if row.get("sales_invoice")]))
    ...
    for row in data:
        ...
        if o_type not in order_types:
            order_types[o_type] = {"amount": 0, "invoices": set()}
